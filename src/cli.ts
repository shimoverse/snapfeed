...
interface ParsedArgs {
  ...
  probe?: string
  /** doctor: run stricter production-readiness checks. */
  prod: boolean
  help: boolean
}

function parseArgs(argv: string[]): ParsedArgs {
  const out: ParsedArgs = { command: undefined, yes: false, prod: false, help: false }
...
function buildEnvExample(choices: InitChoices, existing: string | null): string {
  const header =
    ...
    '# Production guardrails for the server handler:\n' +
    '# SNAPFEED_ALLOWED_ORIGINS=https://staging.example.com,https://app.example.com\n' +
    '# SNAPFEED_RATE_LIMIT_MAX=10\n' +
...
if (process.env.NODE_ENV === 'production' && allowedOrigins.length === 0) {
  throw new Error(
    'SNAPFEED_ALLOWED_ORIGINS must be set before enabling snapfeed in production'
  )
}

const rateLimit = {
  max: Number.parseInt(process.env.SNAPFEED_RATE_LIMIT_MAX ?? '10', 10),
  windowMs: Number.parseInt(process.env.SNAPFEED_RATE_LIMIT_WINDOW_MS ?? '60000', 10),
}
...
Usage:
  ...
  npx snapfeed doctor [--probe=<url>] [--prod]
...
Doctor flags:
      ...
      --prod, --production  Add static production-readiness checks for explicit
                            allowedOrigins and rateLimit guardrails.
